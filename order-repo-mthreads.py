...
def folders_iteration_by_thread(list_files_for_remove, MAIN_PATH_SORT, i):
...
def folders_iteration(MAIN_PATH_SORT, path):
    ...
    # with ThreadPool.ThreadPoolExecutor(max_workers=2) as executor:
    for i in p.glob('**/*'):
        # except_url = str(i).replace('\\', '/')
        ...
        folders_iteration_by_thread(list_files_for_remove, MAIN_PATH_SORT, i)
        thread_folders = Thread(target=folders_iteration_by_thread, args=(list_files_for_remove, MAIN_PATH_SORT, i,))
...
def main_process(path):
    ...
    thread_dict_create = Thread(target=translate_dict_create)
    thread_dict_create.start()
    thread_dict_create.join()
    ...
    res = finish - start
    res_mil = res * 1000
    print(f"Timing of program: {res}")
    print(f"Timing of program mil: {res_mil}")
